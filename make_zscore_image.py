...
def main(tile, ascdesc, stopat):
  ...
  stopdate = (datetime.date(int(stopat[0:4]), int(stopat[4:6]), \
    int(stopat[6:8]))).strftime('%Y%m%d')
  weekdir = 'BottomReflectance/' + startdate+'_to_'+stopdate
  ...
  infile = weekdir + '/' + ascdesc + '/' + tile + '_br.tif'
  ## infile = weekdir + '/' + ascdesc + '/' + tile + '_masked.tif'
