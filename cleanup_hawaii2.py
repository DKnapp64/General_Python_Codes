...
def main(infile):
  ...
  depthfile = '/data/gdcsdata/Research/Researcher/Knapp/Hawaii_Weekly/AscendingDescending/depth_ll' + os.path.sep + tileid + '_depth.tif'
  ## outfile = '/data/gdcsdata/Research/Researcher/Knapp/Hawaii_Weekly/AscendingDescending/Persistent_Deviation/' + os.path.splitext(infile)[0] + '_cleaned.tif'
  ## outfile = '/data/gdcsdata/Research/Researcher/Knapp/Hawaii_Weekly/AscendingDescending/Persistent_Deviation/' + dirparts[-2] + '/' + os.path.splitext(os.path.basename(infile))[0] + '_cleaned.tif'
  outfile = '/data/gdcsdata/Research/Researcher/Knapp/Hawaii_Weekly/AscendingDescending/Persistent_Deviation/' + datepart + '/' + os.path.splitext(os.path.basename(infile))[0] + '_cleaned.tif'
  print("tileID: %s" % (tileid))
  infile = '/data/gdcsdata/Research/Researcher/Knapp/Hawaii_Weekly/AscendingDescending/Persistent_Deviation/' + datepart + '/' + infile
  ...
  with warnings.catch_warnings():
    ...
    masktf = np.logical_or(np.less(mask, 0.0), np.greater(mask,15.0))
...
if __name__ == "__main__":
  ...
  if len( sys.argv ) != 2:
    print("[ USAGE ] you must supply 1 argument: cleanup_hawaii2.py infile")
    sys.exit( 0 )
