...
def main(infile, inaot, inwv, inoz, outputdir):
  ...
  ## mymonth = np.int(os.path.basename(infile)[12:14])
  ## myday = np.int(os.path.basename(infile)[15:17])
  ...
  radmins = np.zeros(5, dtype=np.float32)
  radmaxs = np.zeros(5, dtype=np.float32)
  ...
  rangerededge = np.zeros((2, 256), dtype=np.float32)
  ...
  for lev in np.arange(256):
    ...
    rangerededge[0, lev] = (lev * (radmaxs[3] - radmins[3])/255. + radmins[3]) * 0.01
    rangenir[0, lev] = (lev * (radmaxs[4] - radmins[4])/255. + radmins[4]) * 0.01

  print("Running Iterations of Atmos. Correction")

  for lev in np.arange(256):
    ## create command line string
    cmd1 = "srun -p DGE -n 1 rad_level_rapideye.py "+infile + ((" %d") % (lev)) + (" %6f") % (rangeblue[0,lev])
    cmd2 = (" %6f") % (rangegreen[0,lev]) + (" %6f") % (rangered[0,lev]) + (" %6f") % (rangerededge[0,lev]) + (" %6f") % (rangenir[0,lev])
  ...
  for lev in np.arange(256):
    ...
    rangerededge[1, lev] = inrefl[2] * 10000
    rangenir[1, lev] = inrefl[3] * 10000
  
  np.savez(outputdir+os.path.basename(infile).split('.')[0]+"_luts.npz", rangeblue=rangeblue, rangegreen=rangegreen, rangered=rangered, rangerededge=rangerededge, rangenir=rangenir)
  print("Done!  Luts saved in " + os.path.basename(infile).split('.')[0]+"_luts.npz")
...
if __name__ == "__main__":
  ...
  if len( sys.argv ) != 6:
    print("[ ERROR ] you must supply 4 arguments: generate_rad_to_refl_lut_rapideye.py infile inaot inwv inoz outputdir")
