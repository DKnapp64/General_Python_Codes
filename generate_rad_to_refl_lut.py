...
def main(infile, inaot, inwv, inoz, outputdir):
  ...
  for lev in np.arange(256):
    ...
    cmd1 = "sbatch -q aggressive -n 1 --mem=100 rad_level.py "+infile + ((" %d") % (lev)) + (" %6f") % (rangeblue[0,lev])
  ...
  for lev in np.arange(256):
    ...
    rangeblue[1, lev] = inrefl[0,1] * 10000
    rangegreen[1, lev] = inrefl[1,1] * 10000
    rangered[1, lev] = inrefl[2,1] * 10000
    rangenir[1, lev] = inrefl[3,1] * 10000
    rangeblue[0, lev] = inrefl[0,0]
    rangegreen[0, lev] = inrefl[1,0]
    rangered[0, lev] = inrefl[2,0]
    rangenir[0, lev] = inrefl[3,0]
  
  np.savez(outputdir+os.sep+os.path.basename(infile).split('.')[0]+"_luts.npz", rangeblue=rangeblue, rangegreen=rangegreen, rangered=rangered, rangenir=rangenir)
