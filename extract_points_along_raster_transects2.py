...
for i in range(pntCnt):
  ...
  if (i == 0):
    oldpixval = pixval
    oldlinval = linval
  if (oldpixval == pixval and oldlinval == linval):
    tempvals.append([basd, gasd, rasd])
  else:  
    tarr = np.asarray(tempvals)
    basdm = np.mean(tarr[:,0])
    basdsd = np.std(tarr[:,0])
    gasdm = np.mean(tarr[:,1])
    gasdsd = np.std(tarr[:,1])
    rasdm = np.mean(tarr[:,2])
    rasdsd = np.std(tarr[:,2])
    nvals = len(tempvals)
    f.write(("%s, %s, %3d, %7.4f, %7.4f, %7.4f, %7.4f, %7.4f, %7.4f, %7.2f, %7.4f, %7.4f, %7.4f, %7.4f, %7.4f, %7.4f, %7.4f\n") % (pname, ptime, nvals, basdm, gasdm, rasdm, basdsd, gasdsd, rasdsd, dep, brb, grb, rrb, bsr, gsr, rsr, nsr))
    tempvals = []
    tempvals.append([basd, gasd, rasd])
    oldpixval = pixval
    oldlinval = linval
