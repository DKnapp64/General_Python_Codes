...
if (len(sys.argv) != 2):
  print('make_general_baseline_image2019b.py ascdesc/tileid')
  print('where:')
...
for t,temp in enumerate(blweeks2):
  tempweeks = blweeks.copy()
  tempweeks.extend(blweeks2[0:(t+1)])
  dslist = []
  ## open the data sets for this group
  ## for k,theweek in enumerate(blweeks):
  for k,theweek in enumerate(tempweeks):
    rbtile = theweek + '/' + tile + '_rb_comp.tif'
    if not os.path.exists(rbtile):
      rbtile2 = theweek + '/' + tile + '_rb_comp_masked.tif'
  ...
  outtile = 'JiweiMethod/' + tile + '_' + temp + '_base.tif'
