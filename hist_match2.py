...
in_file1 = '/lustre/scratch/cao/dknapp/Belize_stuff/ortho_vswir/CAO20170414t152532p0000_rgb_ort'
in_file2 = '/lustre/scratch/cao/dknapp/Belize_stuff/ortho_vswir/CAO20170414t152048p0000_rgb_ort'
...
if intersection is not None:
  px1 = [int(np.floor((intersection[0]-r1[0])/gt1[1])), \
         int(np.floor((intersection[1]-r1[1])/gt1[5])), \
         int(np.floor((intersection[2]-r1[0])/gt1[1])), \
         int(np.floor((intersection[3]-r1[1])/gt1[5]))] 
  px2 = [int(np.floor((intersection[0]-r2[0])/gt2[1])), \
         int(np.floor((intersection[1]-r2[1])/gt2[5])), \
         int(np.floor((intersection[2]-r2[0])/gt2[1])), \
         int(np.floor((intersection[3]-r2[1])/gt2[5]))] 
