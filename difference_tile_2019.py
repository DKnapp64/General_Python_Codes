...
for j,name in enumerate(files):
  print('%s  %8s  %8s' % (name, startdate.strftime('%Y%m%d'), stopdate.strftime('%Y%m%d')))
  ...
  if (imgdate < stopdate) and (imgdate >= startdate):
    print('Adding %8s' % (imgdate.strftime('%Y%m%d')))
...
baselinefile = 'BaseFiles2019/'+tileid+'_base.tif'
...
outdir = '/data/gdcsdata/Research/Researcher/Knapp/Hawaii_Weekly/AscendingDescending/Persistent_Deviation/'+stopdate.strftime('%Y%m%d')+'/'
