...
# for Ecuador_Clouds
root_folder_id = '0ByYTB5k_a6rsSEFsbXFkRHo5cDA'
...
while True:
  ...
  # only process during daylight hours in California - check on 15 minute intervals
  if ((datetime.now().hour > 4) and (datetime.now().hour < 19)):
    ...
    while (checktime == 0):
      ...
      try:
        ...
        print 'ERROR: cannot reach satepsanone.nesdis.noaa.gov...will try again in 2 min'
      ...
      for element in filelist:
        if re.search("GoesEast1V[0-3][0-9][0-9][0-2][0-9][0-5][0-9].tif", element):
      ...
      if (file_exists == False):
        ...
        subprocess.call("gdal_translate -of KMLSUPEROVERLAY -projwin -82.0 2.0 -75.0 -6.0 "+\
                        nowfile + " " + kmlname + " -co FORMAT=JPEG", shell=True) 
        print "gdal_translate -of KMLSUPEROVERLAY -projwin -82.0 2.0 -75.0 -6.0 "+ \
      ...
      # sleep for 2 minutes in between checks
      time.sleep(120.0)
