...
infile = 'final_transects_combined.csv'
...
path='transects_combined_25m.shp'
...
for i,thisLine in df.iterrows():
  ...
  if ((float(thisLine[9]) + float(thisLine[4])) == 0):
    feature.SetField('PercBleach', -99)
  else:
    feature.SetField('PercBleach', 100 * float(thisLine[9])/(float(thisLine[9]) + float(thisLine[4])))
  feature.SetField('Compass', float(thisLine[15]))
