...
def zonal_stats(feat, input_zone_polygon, input_value_raster):
    ...
    return featname, featcategory,  len(numpy.ma.compressed(numpy.ma.unique(zoneraster2))), numpy.asscalar(numpy.ma.minimum(zoneraster2).data), numpy.asscalar(numpy.ma.maximum(zoneraster2).data), numpy.count_nonzero(datamask)
...
def loop_zonal_stats(input_zone_polygon, input_value_raster):
    ...
    with open('peru_protected_areas_geoeco_national.csv', 'wb') as csv_file:
...
if __name__ == "__main__":
    ...
    # example run : ~/python_stuff/get_raster_stats_under_geoeco.py anp_nacional_05_08_2016.shp GeoEco_Mar29_2013_PolygonToR12_utm_match
    #
