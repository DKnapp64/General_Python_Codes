...
def main(in1, in2, outf):
  ...
  randnum1 = "%04d" % (randint(0,10000))
  gray1name = "gray1_"+randnum1+".tif"
  grayimg1.save(gray1name, format='TIFF')
  ...
  randnum2 = "%04d" % (randint(0,10000))
  gray2name = "gray2_"+randnum1+".tif"
  grayimg2.save(gray2name, format='TIFF')
  
  del grayimg1, grayimg2, gray1, gray2

  img1 = cv2.imread(gray1name, -1)
  img2 = cv2.imread(gray2name, -1)
...
if __name__ == "__main__":
  ...
  if len( sys.argv ) != 4:
    ...
    print "where:"
    print "    vswirimage = an orthocorrected VSWIR image"
    print "    dimacimage = an orthocorrected DiMAC image or similar 3-band (RGB) image"
    print "    outputfile = an output text file in ENVI image-to-image for warping the VSWIR image to the DiMAC georeferenced space."
    print ""
