    ...
    while (checktime == 0):
      ...
      try:
        ...
        for element in filelist:
          if re.search("GoesWest1V[0-2][0-9][0-9][0-2][0-9][0-5][0-9].tif|GoesWest1V_latest.tif", element):
            tiflist.append(element)
            sizelist.append(ftp.size(element))
    
        ## identify latest TIF file
        ## NOTE: There is an error in the NOAA system that mislabels the time in the file names for the
        ##     Goes images at teh top of the hour.  This hack corrects for that.

        if (sizelist[-1] == sizelist[-2]):
          latesttif = tiflist[-2]
          truename = (os.path.splitext(latesttif))[0]
        else:
          latesttif = tiflist[-3]
          minutes = int(((os.path.splitext(latesttif))[0])[15:17])
          hour = int(((os.path.splitext(latesttif))[0])[13:15])
          day = int(((os.path.splitext(latesttif))[0])[10:13])
          if (hour != 23):
            hour += 1
          else:
            hour = 0
            day += 1
          truename = ((os.path.splitext(latesttif))[0])[0:10] + ('%03d' % day) + ('%02d' % hour) + '00'
          
        file_name_to_check = truename + '_KML.kmz'
        ...
        if (file_exists == False):
          ...
          # download
          print 'Latest TIF at NOAA is: %s' % (latesttif)
          fhandle = open(truename+'.tif', 'wb')
