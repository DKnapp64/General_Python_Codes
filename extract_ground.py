...
def main(inshp, outtxt):
  ...
  imglist2 = glob.glob('/data/gdcsdata/Research/Researcher/Knapp/GEDI/Ground/2**/*_ground.tif')
  imglist3 = glob.glob('/data/gdcsdata/Research/Researcher/Knapp/GEDI/Ground/2**/*_ground.hdr')
  imglist3b = [ n[0:-4] for n in imglist3 ]
  imglist = imglist2 + imglist3b
  print('Using %d Ground images to compare against.' % (len(imglist)))
  ...
  ## fout.write('ShotID, ImageName, FCov1, fCov2, MeanTCH1, SdevTCH1, ModeTCH1, NTCH1MeanTCH2, SdevTCH2, ModeTCH2, NTCH2\n')
  ...
  for feat in lyr:
    ...
    for k,img in enumerate(covlist):
      ...
      good1 = np.logical_and(np.greater(mydata1, -9999), np.less(mydata1, 100000.0))
      good2 = np.logical_and(np.greater(mydata2, -9999), np.less(mydata2, 100000.0))
...
if __name__ == "__main__":
  ...
  if len( sys.argv ) != 3:
    print("[ USAGE ] you must supply 2 arguments: extract_ground.py inshapefile outtext")
    print("")
    sys.exit( 1 )
