...
def main(inshp, outtxt):
  ...
  ## diam1 = 25.
  ## diam2 = 50.
  ## for resolution 1.12m
  ## outsize1 = ceil(25./1.12)
  ## circpnts1 = np.zeros((outsize1,outsize1), dtype=np.bool)
  ## centx = outsize1/2.; centy = outsize1/2.;
  ## for i in np.arange(0.5, outsize1+0.5, 1): 
  ##   for j in np.arange(0.5, outtsize1+{0.5, 1):
  ...
  imglist2 = glob.glob('/data/gdcsdata/Research/Researcher/Knapp/GEDI/Ground/2**/*_slope.tif')
  ## imglist3 = glob.glob('/data/gdcsdata/Research/Researcher/Knapp/GEDI/Ground/2**/*_slope.hdr')
  ## imglist3b = [ n[0:-4] for n in imglist3 ]
  ## imglist = imglist2 + imglist3b
  imglist = imglist2
  print('Using %d Slope images to compare against.' % (len(imglist)))
  cornarr = np.zeros((len(imglist), 4))

  for k,img in enumerate(imglist):
    tmpDS = gdal.Open(img, gdal.GA_ReadOnly)
    if (tmpDS is None):
      print('File %s did not open' % (img))
      continue
  ...
  fout = open(outtxt, 'w')
  fout.write('ShotID, ImageName, Res, MeanSLP1, SdevSLP1, ModeSLP1, NSLP1, MeanSLP2, SdevSLP2, ModeSLP2, NSLP2\n')
  ...
  for feat in lyr:
    ...
    for k,img in enumerate(covlist):
      ...
      if math.isclose(mygt[1], 1.12, abs_tol=0.01) or math.isclose(mygt[1], 0.00, abs_tol=0.01):
        print('Problem image %s: %5.2f resolution.' % (img, mygt[1]))
        inDS = None
        continue 
      ...
      good1 = np.logical_and(np.greater(mydata1, -9999), np.less(mydata1, 90.0))
      good2 = np.logical_and(np.greater(mydata2, -9999), np.less(mydata2, 90.0))
      ...
      if (myn1 == 0) or (myn2 == 0):
        ...
        fout.write('%18d, %s, %6.2f, %8.2f, %8.2f, %8.2f, %d, %8.2f, %8.2f, %8.2f, %d\n' % (shotnum, img, mygt[1], mymean1, mysdev1, mymode1, myn1, mymean2, mysdev2, mymode2, myn2))
        ## fout.write('%18d, %s, %6.2f, %8.1f, %8.1f, %8.2f, %8.2f, %8.2f, %8.2f, %d, %8.2f, %8.2f, %8.2f, %8.2f, %d\n' % (shotnum, img, mygt[1], fcov1, fcov2, mymean1, mysdev1, mymode1, myn1, mymean2, mysdev2, mymode2, myn2))
        inDS = None
...
if __name__ == "__main__":
  ...
  if len( sys.argv ) != 3:
    print("[ USAGE ] you must supply 2 arguments: extract_slope.py inshapefile outtext")
    print("")
    sys.exit( 1 )
