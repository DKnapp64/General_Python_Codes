...
vswirfile = '/lustre/scratch/cao/dknapp/Kaneohe/patch44_20170905_rb_warped'
dicamfile = '/lustre/scratch/cao/dknapp/Kaneohe/Reef_44_20170905A_minpath_mosaic_bal_envi_warped_trimmed'
maskfile = '/lustre/scratch/cao/dknapp/Kaneohe/patch44_20170930_rb_mask'
hyperfile = 'patch44_20170905_hybrid_rb'
...
bandvswir = np.zeros((ycountvswir, xcountvswir)).astype(np.float)
banddicam = np.zeros((ycountdicam, xcountdicam)).astype(np.float)

points = np.array((row, col)).T
grid_x, grid_y = np.mgrid[0:ycountvswir:np.complex(ycountdicam),0:xcountvswir:np.complex(xcountdicam)]
myQ100 = np.zeros((goodbig.sum(), vswir.RasterCount+1))

for c in range(vswir.RasterCount+1):
  values = myQ[:,c]
  gridband = griddata(points, values, (grid_x, grid_y), method='linear') 
  myQ100[:,c] = gridband[rowbig, colbig]
  print "Finished band %d of %d" % (c+1, vswir.RasterCount+1)
...
del dicamdata, myQ, vswirgood, comboarr
