...
with PdfPages('baseline_bleaching_diff_compare_20191129.pdf') as pdf:
  for k,numpv in enumerate(thevals):
    fig = plt.figure(figsize=(10,8))
    ax = plt.subplot(1, 1, 1)
    ax.set_title('Hawaii Weekly Refl. Difference')
    ax.set_ylabel('Refl. Difference (%)')
    ax.xaxis.set_major_locator(loc)
    ax.xaxis.set_major_formatter(formatter)
    ax.xaxis.set_tick_params(rotation=30, labelsize=7)
    ## plt.errorbar(thedates, np.repeat(basearr[k,0]/100.0, len(thedates)), yerr=np.repeat(basearr[k,1]/100.0, len(thedates)), fmt='-b', capsize=5, label='Baseline')
    plt.errorbar(thedates, np.repeat(0.0, len(thedates)), yerr=np.repeat(basearr[k,1]/100.0, len(thedates)), fmt='-b', capsize=5, label='Baseline')
    plt.errorbar(thedates, bigarr[:,k,0]/100.0, yerr=bigarr[:,k,1]/100.0, fmt='--r', capsize=5, label='PV %d'%(numpv))
