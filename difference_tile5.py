...
baseline_files = [x for x in files if '201908' not in x and '201909' not in x and '201910' not in x and '201911' not in x and '201912' not in x]
temp_bleaching_files = [x for x in files if '201908' in x or '201909' in x or '201910' in x or '201911' in x or '201912' in x]
bleaching_files = []
...
outname = 'persistant_below_{}_{}_{}.tif'.format(ascdesc, tileid, stopdate.strftime('%Y%m%d'))
...
for _line in range(y_off, y_off+y_size):
    ...
    for _f in range(len(bleaching_datasets)):
        ...
        # BELOW BASELINE
        # add one for each deviation BELOW baseline
