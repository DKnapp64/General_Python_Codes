...
def main(infile, outfile, chla_global):
  ...
  root = os.path.basename(infile)[0:15]
  depthfile = 'depth_ll/southchinasea_dem.vrt'
  ## depthfile = 'depth_ll/eastern_hawaii_sentinel2.tif'
  ...
  if not os.path.exists(infile):
    newname = os.path.dirname(infile) + os.path.sep + root + '.tif'
    if os.path.exists(newname):
      infile = newname
      print("Namechange: %s" % (infile))
...
if __name__ == "__main__":
  ...
  if len( sys.argv ) != 4:
    print("[ USAGE ] you must supply 3 arguments: depth_rb_noauto.py infile outfile chla")
