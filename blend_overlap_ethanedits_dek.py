...
def main(inlistfile, outname):
  #inlistfile = "test_blend_list"
  #outname = "build/orthoed_frames/test_two_frame_mosaic"
  inlist  = [line.rstrip() for line in open(inlistfile, "r")]
  randroot = "temp" + ("%05d" % (np.random.randint(0,99999)))
  stitcher = cv2.createStitcher(False)
  #foo = np.uint8(cv2.imread(inlist[0]).T)
  #bar = np.uint8(cv2.imread(inlist[1]).T)
  #foo1 = np.uint8(cv2.imread(inlist[2]).T)
  #bar1 = np.uint8(cv2.imread(inlist[3]).T)
  
  #img1 = cv2.cvtColor(foo,cv2.COLOR_BGR2GRAY)
  #img2 = cv2.cvtColor(bar,cv2.COLOR_BGR2GRAY)
  
  #result = stitcher.stitch((foo,bar,foo1,bar1))
  ...
  for i in np.arange(len(inlist)-1):
    ...
    print("ulx1 uly1 lrx1 lry1: %d %d %d %d" % (ulx1, uly1, lrx1, lry1))
    print("ulx2 uly2 lrx2 lry2: %d %d %d %d" % (ulx2, uly2, lrx2, lry2))
    ...
    comboxsize = np.floor((lrxcombo - ulxcombo)/geo1[1]).astype(np.int)
    comboysize = np.floor((ulycombo - lrycombo)/geo1[1]).astype(np.int)
    
    combo = np.zeros((2, comboysize, comboxsize), dtype=np.uint8)
    newgt = (ulxcombo, geo1[1], geo1[2], ulycombo, geo1[4], geo1[5])
    
    #print("ulxcombo ulycombo lrxcombo lrycombo: %d %d %d %d" % (ulxcombo, ulycombo, lrxcombo, lrycombo))
    #print("comboxsize comboysize combo newgt: %d %d %d %d" % (comboxsize, comboysize, combo, newgt))
    ...
    print("width1 height1 startx1 starty1 stopx1 stopy1: %d %d %d %d %d %d" % (width1, height1, startx1, starty1, stopx1, stopy1))
    print("width2 height2 startx2 starty2 stopx2 stopy2: %d %d %d %d %d %d" % (width2, height2, startx2, starty2, stopx2, stopy2))


    gray1 = np.uint8(cv2.cvtColor(np.transpose(temp1[:, slice(starty1, stopy1), slice(startx1, stopx1)], (1,2,0)), cv2.COLOR_BGR2GRAY)) 
    gray2 = np.uint8(cv2.cvtColor(np.transpose(temp2[:, slice(starty2, stopy2), slice(startx2, stopx2)], (1,2,0)), cv2.COLOR_BGR2GRAY))
    ...
    outDS4 = driver.Create(newref, xsize=comboxsize, ysize=comboysize, bands=3, eType=gdal.GDT_UInt16)
    ...
    print(" yoff1 yoff2: %d %d" % (yoff1, yoff2))
    print(" xoff1 xoff2: %d %d" % (xoff1, xoff2))
    print("overlap y")
    print(overlapy)
    print("overlap x")
    print(overlapx)
    print("Props0 overlapy overlapx")
    print(props[0,overlapy,overlapx])
    print("Props1 overlapy overlapx")
    print(props[1,overlapy,overlapx])


    for f in np.arange(3):
      outband = np.zeros((comboysize,comboxsize), dtype=np.uint16)
      outband[combo1rows, combo1cols] = temp1[f,good1rows,good1cols]
      outband[combo2rows, combo2cols] = temp2[f,good2rows,good2cols]
      ## check indices
      temp1_heightcheck  = np.logical_and(np.less(overlapy-yoff1, temp1.shape[1]), np.greater_equal(overlapy-yoff1, 0))
      temp1_widthcheck  = np.logical_and(np.less(overlapx-xoff1, temp1.shape[2]), np.greater_equal(overlapx-xoff1, 0))
      props_heightcheck  = np.logical_and(np.less(overlapy, props.shape[1]), np.greater_equal(overlapy, 0))
      props_widthcheck  = np.logical_and(np.less(overlapx, props.shape[2]), np.greater_equal(overlapx, 0))
      out_heightcheck  = np.logical_and(np.less(overlapy, outband.shape[0]), np.greater_equal(overlapy, 0))
      out_widthcheck  = np.logical_and(np.less(overlapx, outband.shape[1]), np.greater_equal(overlapx, 0))
      ## find where indices are all within bounds
      all_good = np.all(np.stack((temp1_heightcheck, temp1_widthcheck, props_heightcheck, props_widthcheck, out_heightcheck, out_widthcheck), axis=-1), axis=1)
      ## remove indices that are out of bounds
      overlapy, overlapx = overlapy[all_good], overlapx[all_good]
      ## apply blend with updated indices
